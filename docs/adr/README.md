...
41. [ADR-041: Establish TURNLOCK formal assurance architecture](adr-041-establish-turnlock-formal-assurance-architecture.md) — **Accepted**, amends ADR-015 and confirms ADR-040. Formal assurance gains an explicit architecture: normative authority stays with the specification and accepted ADRs; required assurance claims (`TL-CLAIM-*`) decompose invariants before backend realization; the integrated TLA+ semantics becomes the initial canonical formal semantic representation after hostile semantic review; claim/property correspondence and other non-mechanical semantic boundaries require durable adversarial review evidence distinct from mechanical checker evidence; and readiness is staged into Gate A (candidate formal-model authoring), Gate B (canonical-formal-semantic promotion), and Gate C (verification-evidence lifting). No executable model, TLA+ identifier, new invariant, product semantic, or generic IR is introduced.
42. [ADR-042: Define auditable hostile-review campaign execution and adjudication](adr-042-define-auditable-hostile-review-campaign-execution-and-adjudication.md) — **Accepted**, clarifies ADR-041. The hostile-review evidence contract is strengthened without changing TURNLOCK product semantics: independence means operationally independent executions, with distinct `(provider, model, model_version)` identities counting toward the independent-reviewer minimum and provider diversity recommended but not universally required; each qualifying execution must cover the complete required attack-objective set and use the same exact canonical packet and prompt; raw reviewer outputs are sealed and content-addressed; every declared raw finding must map exactly once into the normalized finding ledger; materiality is derived from explicit impact axes rather than stored as a free boolean; refutations must be structured and every material refutation requires a sealed hostile challenge; and current material `open`, `routed`, or `resolved` findings block Gate A while invalid review evidence forces Gate A BLOCKED. The Gate A semantic subject is unchanged and ADR-042 is not a semantic dependency of it.
43. [ADR-043: Bind hostile-review evidence to exact reviewed inputs](adr-043-bind-hostile-review-evidence-to-exact-reviewed-inputs.md) — **Accepted**, amends ADR-042 and confirms ADR-041. Rejects three assurance defects in the hostile-review evidence contract: a Gate A derived-subject campaign could supply an arbitrary content-addressed review packet that was never bound to the reviewed subject; a material refutation could change after its challenge was produced because the challenge was not bound to the exact refutation payload; and repository-relative artifact paths could resolve through symlinks. ADR-043 binds Gate A review packets to exact self-contained reviewed subjects, binds hostile challenges to exact refutations through `challenged_refutation_sha256`, and requires direct non-symlink repository-contained artifact identity. It changes review-evidence validity only and does not change the Gate A semantic subject.
44. [ADR-044: Require one unambiguous Gate A review subject](adr-044-require-one-unambiguous-gate-a-review-subject.md) — **Accepted**, amends ADR-043 and confirms ADR-042. ADR-044 closes a subject-confusion defect in the Gate A hostile-review evidence contract: a Gate A assurance-decomposition review record that declares `gate-a-assurance-decomposition-v1` must contain exactly one Gate A derived subject, and packet binding plus Gate A currentness must both use that same unique subject identity. Duplicate or multiple distinct Gate A derived subjects are invalid evidence, with no first-wins, last-wins, packet-wins, or current-wins precedence; additional artifact subjects remain permitted as long as the canonical packet is bound to the unique Gate A derived subject. The review-evidence schema remains version 3.0 and the Gate A semantic subject is unchanged.
45. [ADR-045: Bind Gate A campaigns to a versioned review protocol and derived evidence](adr-045-bind-gate-a-campaigns-to-versioned-review-protocol-and-derived-evidence.md) — **Accepted**, amends ADR-042, ADR-043, and ADR-044 and confirms ADR-041. ADR-045 strengthens Gate A hostile-review assurance without changing TURNLOCK product semantics. It separates the reviewed semantic subject from a versioned hostile-review protocol identity; binds current campaigns to both; introduces protocol-qualified reviewer profiles, evidence-derived model-version resolution, structured JSON raw outputs, sealed execution receipts, one-to-one finding normalization, conservative materiality/refutation challenges, stale-protocol finding re-adjudication, and exact external runner outcomes. A protocol change cannot erase a surviving finding, failed refutation is not proof of finding truth, and DECISION-REQUIRED is reserved for genuine product underdetermination or authority conflict. The Gate A semantic subject remains unchanged.
46. [ADR-046: Bind challenge executions to exact inputs and derive retry admissibility](adr-046-bind-challenge-executions-to-exact-inputs-and-derive-retry-admissibility.md) — **Accepted**, amends ADR-043 and ADR-045 and confirms ADR-042. ADR-046 closes two evidence-integrity bypasses and one protocol-durability gap without changing TURNLOCK semantics: attempt validity is now independently derived from sealed output so a valid semantic result cannot be mislabeled and retried; hostile challenge executions are bound to canonical self-contained challenge packets containing the exact challenged candidate and reviewed Gate A packet; and protocol evolution becomes append-only through a recursively validated predecessor chain. Review evidence advances to schema 5.0 and the current protocol to gate-a-campaign-protocol-v2 while the Gate A semantic subject remains unchanged.
47. [ADR-047: Make unvalidated cognitive completions terminal and validate readiness projections](adr-047-make-unvalidated-cognitive-completions-terminal-and-validate-readiness-projections.md) — **Accepted**, amends ADR-046 and confirms ADR-041 and ADR-045. ADR-047 closes the remaining completed-response retry ambiguity without inventing output schemas for adjudication roles. Protocol v3 distinguishes initial-reviewer and challenge, whose output validity is mechanically derivable, from the seven cognitive roles without deterministic validators; for those roles, technical failures may retry but the first completed response is the terminal qualified completion and protocol-invalid is forbidden. Published protocol-v2 artifacts become immutable regression anchors, and the generated formal readiness mapping now depends on a full successful traceability/evidence validation pass rather than calling derive_gate_a() directly. TURNLOCK product semantics and the Gate A semantic subject remain unchanged.
48. [ADR-048: Content-address hostile-review meta-schemas](adr-048-content-address-hostile-review-meta-schemas.md) — **Accepted**, amends ADR-045, ADR-046, and ADR-047 and confirms ADR-041. ADR-048 closes the remaining interpretation dependency outside content-addressed hostile-review evidence. The previously mutable global review-evidence and protocol-bundle schemas become frozen legacy aliases; immutable versioned meta-schema snapshots are published under `formal/reviews/meta-schemas/`. Gate A campaign protocol v4 binds the exact protocol-bundle meta-schema and review-evidence meta-schema into protocol identity `P`, while v1-v3 remain interpretable through the immutable historical snapshot. The checker remains the finite mechanical schema-selection root, avoiding an infinite meta-schema registry chain. Review evidence remains schema 5.0, execution receipts remain schema 3.0, and the Gate A semantic subject is unchanged.
49. [ADR-049: Clarify hostile-review execution receipt attempt mapping](adr-049-clarify-hostile-review-execution-receipt-attempt-mapping.md) — **Accepted**, amends ADR-045, ADR-046, and ADR-047 and confirms ADR-048. ADR-049 resolves the execution-receipt/call ambiguity without changing execution-receipt schema 3.0: one receipt represents one logical cognitive protocol execution, `attempts[]` records its ordered protocol attempts, each protocol attempt owns one `call_id`, and provider/transport retries remain internal to that call through `transport_attempt_count`. An admissible protocol retry adds another attempt to the same receipt; the first qualified attempt remains final. A hostile challenge remains a distinct logical execution with its own receipt.
50. [ADR-050: Allow shared governance implementation without transferring repository authority](adr-050-allow-shared-governance-implementation-without-transferring-repository-authority.md) — **Accepted**, amends ADR-017. ADR-050 separates repository authority from reusable governance implementation: TURNLOCK keeps product semantics, accepted decision history, local profiles and bindings, evidence, and generated outputs, while immutable-pinned proto-ring tooling may own shared enforcement mechanisms after parity is demonstrated. Mutable provider state cannot become ambient validation authority, and the existing external OKF schema authority is unchanged.
